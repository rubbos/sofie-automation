...
class MainForm(FlaskForm):
    ...
    places_of_residence = FieldList(FormField(ResidenceForm), min_entries=1)
    nl_residence_dates = FieldList(FormField(DateRangeForm), min_entries=1)
    nl_deregister_date = DateField('Deregister date', format='%Y-%m-%d', render_kw={"type": "date"}, validators=[DataRequired()])
    nl_private_dates = StringField('NL private dates')
    nl_dutch_employer_dates = StringField('NL worked for dutch employer')
    nl_worked_dates = StringField('NL worked dates')
    ...
    explain_will_agreement = StringField('Explain wilsovereenkomst')
    previous_jobs = StringField('Previous jobs', validators=[DataRequired()])
    nl_explain = StringField('Explain periods in NL')
