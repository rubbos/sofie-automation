...
if not os.path.isfile(f"extracted_{file_path}.txt"):
    ...
    with open(f"extracted_{file_path}.txt", "w") as text_file:
        text_file.write(text)

# If file exists, skip the pytesseract to speedup testing
else:
    with open(f"extracted_{file_path}.txt", "r") as text_file:
        text = text_file.read()


# Extract the data between 2 keywords, refactor needed
...
# Extract multiple place, country with dates
...
place_location = extract_text_between_keywords(
    place_location, keyword_place_location_specified, "Date"
)
place_location.replace(":", "")
place_location.replace("Country", "")
